...
def generate_grid(min_ca: str, max_ca: str, min_cb: str, max_cb: str,
    ...
    for i in range(resolution):
        for j in range(resolution):
            if resolution > 1:
                ca = min_ca_dec + (max_ca_dec - min_ca_dec) * Decimal(i) / \
                    Decimal(resolution - 1)
                cb = min_cb_dec + (max_cb_dec - min_cb_dec) * Decimal(j) / \
                    Decimal(resolution - 1)
            else:
                ca = min_ca_dec
                cb = min_cb_dec
...
class MandelbrotWorker:
    ...
    Each worker maintains a persistent subprocess to avoid process spawn
    overhead. Communication is handled via stdin/stdout.
    """
    ...
    def calculate(self, precision: int, za: str, zb: str, ca: str, cb: str,
        ...
        with self.lock:
            # Send CAL command
            cmd = (f"CAL {precision} {za} {zb} {ca} {cb} "
                   f"{max_iterations} {escape_radius}\n")
...
class MandelbrotPool:
    ...
    This class manages a collection of MandelbrotWorker instances,
    distributing tasks via a queue and collecting results.
    """
    def __init__(self, mandelbrot_path: str, num_workers: int = None):
        if num_workers is None:
            num_workers = cpu_count()
        self.workers = [MandelbrotWorker(mandelbrot_path)
    ...
    def _worker_thread(self, worker: MandelbrotWorker):
        ...
        while self.running:
            try:
                ...
                if task is None:
                    break
                (idx, precision, za, zb, ca, cb,
                 max_iterations, escape_radius) = task
                result = worker.calculate(
                    precision, za, zb, ca, cb, max_iterations, escape_radius)
                result['idx'] = idx
                result['ca'] = ca
                result['cb'] = cb
    ...
    def submit(self, idx: int, precision: int, za: str, zb: str, ca: str,
               cb: str, max_iterations: int, escape_radius: str):
        """Submit a calculation task."""
        self.task_queue.put(
            (idx, precision, za, zb, ca, cb, max_iterations, escape_radius))
...
def calculate_mandelbrot_grid(min_ca: str, max_ca: str, min_cb: str,
                              max_cb: str, resolution: int,
                              start_max_iterations: int, escape_radius: str,
                              output_path: str):
    """
    ...
    if not os.path.exists(mandelbrot_path):
        ...
        sys.exit(1)
    # Calculate precision
    precision = calculate_precision(min_ca, max_ca, min_cb, max_cb,
                                    resolution)
    print(f"Using precision: {precision} bits", file=sys.stderr)
    # Generate grid
    grid = generate_grid(min_ca, max_ca, min_cb, max_cb, resolution)
    total_points = len(grid)
    print(f"Grid size: {resolution}x{resolution} = {total_points} points",
          file=sys.stderr)
    ...
    while True:
        ...
        unescape_indices = [
            ...
            if results[idx]['escaped'] == 'N' and
            results[idx]['iterations'] < max_total_iterations
        ]
        if not unescape_indices:
            print("All points processed", file=sys.stderr)
            break
        print(f"Iteration round: max_iterations={max_iterations}, "
              f"processing {len(unescape_indices)} points",
              file=sys.stderr)
    ...
    with open(output_path, 'w', newline='') as csvfile:
        fieldnames = ['CA', 'CB', 'ESCAPED', 'ITERATIONS',
                      'FINAL_ZA', 'FINAL_ZB']
...
def main():
    if len(sys.argv) != 9:
        print("Usage: box_calculator.py <min_ca> <min_cb> <max_ca> <max_cb> "
              "<resolution> <start_max_iterations> <escape_radius> "
              "<output_path>",
              file=sys.stderr)
        sys.exit(1)
    min_ca = sys.argv[1]
    ...
    calculate_mandelbrot_grid(min_ca, max_ca, min_cb, max_cb, resolution,
                              start_max_iterations, escape_radius,
                              output_path)
